...
- **[口播]** 如何用 Vibe Coding，快速搞定自动化视频渲染？先记住一个关键认知：AI 最能打的本事，是啃文本和代码。所以想让渲染自动化，思路就一条——把视频变成代码和数据，让 AI 按配置出片。改一行配置，整条片子自动重渲。这就是低成本、高效率的自动化生产。怎么落地？三步：先让 AI 找技术路径，再对着实际需求选型，最后落地出片。
...
- **[口播]** 第一步，找技术路径。别自己一个个去查，直接问 AI：用 AI 编程做自动化渲染，现在有哪些现成路子？它一口气摆了六条——Remotion 走网页那套，Manim 画数学公式，MoviePy、FFmpeg 做拼接，还有 Motion Canvas、PixiJS。名字五花八门，可你扒到底，全在干同一件事：拿代码描述画面，编译成一帧一帧，再合成视频。六条路摆出来了。到底选哪条？
...
- **[口播]** 第二步，技术选型。别急着问 AI 哪个好——它会把每个都夸一遍，你照样选不出来。想让它帮你拍板，就让它做一次全方位对比：每条路的核心优势、劣势，全摆到台面上，列成一张表。就说 Remotion。优势很实在：前端 React 栈、排版能力强、模板还能跨期复用。劣势它也不藏——组件顶层直接读浏览器对象，打包阶段当场崩；而且是 BUSL 授权，做大了要给钱。优劣摆齐，再拿自己的约束去卡。我的约束就三条：一个固定模板、换数据批量出几十期；AI 改内容别老出错；跨期还得好维护。三条一卡，Remotion 胜出。你可能会问，直接拿 HTML 画页面不行吗？行。但你改一处，剩下几十期不会跟着变。Remotion 不一样，改一处全系列生效，AI 只能照着填数据，动不了结构。十期之后，照样管得住。记住一点：AI 负责把优劣铺开；怎么权衡、选哪条，这个判断得你自己下。
...
- **[口播]** 路线定了 Remotion。它怎么工作？用 React 写页面和动画，引擎逐帧截图，再合成视频。你写的就是一个网页——网页上能画的，它就能渲成片。而且从页面到成品全自动，不用你盯着，也不用手动剪。那它为什么这么好被 AI 驱动？我总结了四条。写的就是网页——React 加 CSS 是 AI 语料里最厚的一块，最熟也最不容易错。内容和模板分开，改数据就改片，正好撞在它改文本的强项上。字段还用 TypeScript 定死，填错漏填当场报红，乱来的空间压到最小。还有，出片全自动，截图、合成全交给引擎，天生适合批量。一条心法：把视频写成数据，AI 只负责填空，不乱跑。原理通了，接着搭引擎。
...
- **[口播]** 选型定了，进落地。引擎不用你自己写，你要做的，是把配置和现成组件对上号。这台引擎叫 remotion-composer。你写一份配置，讲清这段画面是什么；主程序 Explainer 读配置里的 type 字段，自动挑对应组件去渲。现成的模板场景有一整套——开场、收尾、概念、要点、流程、表格、图表对比，还有合成终端和截图叠层。全是深色科技底加白字，改一处主题，全片跟着变。所以做内容，其实就是挑组件、填字段。想要一张对比卡？你就说左边传统剪辑、右边代码即视频，AI 吐出来的，就是一段 type 写 comparison 的配置，齐活。这里有条铁律：不许 AI 发明新组件，只让它照现成的填数据。让它从零手写，既在重复造轮子，又把「换数据就复用」的好处弄没了。而且每个字段都用 TypeScript 定死，AI 填错、漏填，当场报红，想乱发挥都没空间。至于在现成组件上再扩一套自有品牌风格，那是更大的话题，以后单独开一期。这期，先用现成的把片子跑通。
...
- **[口播]** 视频要不要一个出镜形象来串场？先把定位钉死：数字人只是陪衬，不是主角，真正的干货，永远是屏幕上的真实操作。定位定了，套一样的方法论——让 AI 把每种形象的优势、劣势全方位摆出来。真人出镜最可信、有温度，但你得露脸，也没法编程批量复用。写实数字人对口型像真主播，可它太容易掉进恐怖谷，一掉进去，可信度就崩了。二次元或者 3D 风格化角色走 VRM，风格统一、能编程、渲一次到处用，还不踩恐怖谷。优劣摆全，再拿约束去卡——不露脸、要能编程批量复用、避开恐怖谷。选定 3D 风格化角色 VRMAvatar，并且定死一条：只做陪衬，坚决不做对口型。落地照样交给 AI：整体渲一次，再按场景裁——角落、半身、全身。
...
```json
{
  ...
  "sections": [
      ...
      "shots": [
        {
          ...
          "voice_slice": "如何用 Vibe Coding，快速搞定自动化视频渲染？",
          "duration_seconds": 6
        },
        {
          "id": "1.2",
          ...
          "voice_slice": "先记住一个关键认知：AI 最能打的本事，是啃文本和代码。所以想让渲染自动化，思路就一条——把视频变成代码和数据，让 AI 按配置出片。",
          "duration_seconds": 10
        },
        {
          "id": "1.3",
          ...
          "voice_slice": "改一行配置，整条片子自动重渲。这就是低成本、高效率的自动化生产。",
          "duration_seconds": 10
        },
        {
          "id": "1.4",
    ...
    },
    {
      "id": "2",
      "track": "A",
      "voice": "第一步，找技术路径。别自己一个个去查，直接问 AI：用 AI 编程做自动化渲染，现在有哪些现成路子？它一口气摆了六条——Remotion 走网页那套，Manim 画数学公式，MoviePy、FFmpeg 做拼接，还有 Motion Canvas、PixiJS。名字五花八门，可你扒到底，全在干同一件事：拿代码描述画面，编译成一帧一帧，再合成视频。六条路摆出来了。到底选哪条？",
      ...
      "shots": [
        {
          ...
          "voice_slice": "第一步，找技术路径。别自己一个个去查，直接问 AI：用 AI 编程做自动化渲染，现在有哪些现成路子？",
          "duration_seconds": 7
        },
        {
          "id": "2.2",
          ...
          "voice_slice": "它一口气摆了六条——Remotion 走网页那套，Manim 画数学公式，MoviePy、FFmpeg 做拼接，还有 Motion Canvas、PixiJS。",
          "duration_seconds": 12,
          "visual_beats": [
            {
              "at_seconds": 0,
        ...
        },
        {
          ...
          "voice_slice": "名字五花八门，可你扒到底，全在干同一件事：拿代码描述画面，编译成一帧一帧，再合成视频。六条路摆出来了。到底选哪条？",
          "duration_seconds": 9
        }
      ]
    },
      ...
      "shots": [
        {
          ...
          "voice_slice": "第二步，技术选型。别急着问 AI 哪个好——它会把每个都夸一遍，你照样选不出来。想让它帮你拍板，就让它做一次全方位对比：每条路的核心优势、劣势，全摆到台面上，列成一张表。",
          "duration_seconds": 16,
          "visual_beats": [
            {
              "at_seconds": 0,
        ...
        },
        {
          ...
          "voice_slice": "优劣摆齐，再拿自己的约束去卡。我的约束就三条：一个固定模板、换数据批量出几十期；AI 改内容别老出错；跨期还得好维护。三条一卡，Remotion 胜出。",
          "duration_seconds": 14
        },
        {
          "id": "3.4",
        ...
        },
        {
          ...
          "props": {
            "text": "AI 负责把优劣铺开；怎么权衡、选哪条，得你自己下。"
          },
          "voice_slice": "记住一点：AI 负责把优劣铺开；怎么权衡、选哪条，这个判断得你自己下。",
          "duration_seconds": 6
        }
      ]
    },
    {
      "id": "4",
      "track": "A",
      "voice": "路线定了 Remotion。它怎么工作？用 React 写页面和动画，引擎逐帧截图，再合成视频。你写的就是一个网页——网页上能画的，它就能渲成片。而且从页面到成品全自动，不用你盯着，也不用手动剪。那它为什么这么好被 AI 驱动？我总结了四条。写的就是网页——React 加 CSS 是 AI 语料里最厚的一块，最熟也最不容易错。内容和模板分开，改数据就改片，正好撞在它改文本的强项上。字段还用 TypeScript 定死，填错漏填当场报红，乱来的空间压到最小。还有，出片全自动，截图、合成全交给引擎，天生适合批量。一条心法：把视频写成数据，AI 只负责填空，不乱跑。原理通了，接着搭引擎。",
      ...
      "shots": [
        ...
        },
        {
          ...
          "voice_slice": "那它为什么这么好被 AI 驱动？我总结了四条。写的就是网页——React 加 CSS 是 AI 语料里最厚的一块，最熟也最不容易错。内容和模板分开，改数据就改片，正好撞在它改文本的强项上。字段还用 TypeScript 定死，填错漏填当场报红，乱来的空间压到最小。还有，出片全自动，截图、合成全交给引擎，天生适合批量。",
          "duration_seconds": 16
        },
        {
          "id": "4.3",
    ...
    },
    {
      ...
      "shots": [
        ...
        },
        {
          ...
          "voice_slice": "现成的模板场景有一整套——开场、收尾、概念、要点、流程、表格、图表对比，还有合成终端和截图叠层。全是深色科技底加白字，改一处主题，全片跟着变。",
          "duration_seconds": 14
        },
        {
          "id": "5.3",
    ...
    },
    {
      "id": "6",
      "track": "A",
      "voice": "视频要不要一个出镜形象来串场？先把定位钉死：数字人只是陪衬，不是主角，真正的干货，永远是屏幕上的真实操作。定位定了，套一样的方法论——让 AI 把每种形象的优势、劣势全方位摆出来。真人出镜最可信、有温度，但你得露脸，也没法编程批量复用。写实数字人对口型像真主播，可它太容易掉进恐怖谷，一掉进去，可信度就崩了。二次元或者 3D 风格化角色走 VRM，风格统一、能编程、渲一次到处用，还不踩恐怖谷。优劣摆全，再拿约束去卡——不露脸、要能编程批量复用、避开恐怖谷。选定 3D 风格化角色 VRMAvatar，并且定死一条：只做陪衬，坚决不做对口型。落地照样交给 AI：整体渲一次，再按场景裁——角落、半身、全身。",
      ...
      "shots": [
        ...
        },
        {
          ...
          "voice_slice": "定位定了，套一样的方法论——让 AI 把每种形象的优势、劣势全方位摆出来。真人出镜最可信、有温度，但你得露脸，也没法编程批量复用。写实数字人对口型像真主播，可它太容易掉进恐怖谷，一掉进去，可信度就崩了。二次元或者 3D 风格化角色走 VRM，风格统一、能编程、渲一次到处用，还不踩恐怖谷。",
          "duration_seconds": 18
        },
        {
          "id": "6.3",
